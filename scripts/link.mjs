...
const symlinkPath = resolve(dataPath, "systems", systemJSON.id);
...
if (symlinkStats) {
	...
	const proceed = (
		await prompts({
			...
			name: "value",
			initial: false,
			message: `A "${systemJSON.id}" ${atPath} already exists in the "systems" subfolder. Replace with new symlink?`,
		})
	).value;
	if (!proceed) {
